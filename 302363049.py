...
class HuffmanCoding(Huffman_code_interface.HuffmanCoding):  # This is the way you construct a class that inherits properties
           ...
           :rtype: A: dictionary"""
            if self.given_file_type == "bin":
                ...
                for binary_number in self.file_word_string:
                    self.file_word_list.append(binary_number)
    ...
    def decompress_file(self, input_file_path):
        ...
        def write_to_decompressed_file(decompressed_path, element_list):
            if decompressed_path.endswith('.bin'):
                with open(decompressed_path, 'wb') as file:
                    arr = bytearray(element_list)
                    file.write(arr)
            else:
        ...
        def extract_binary_string_from_file(input_file_path):
        ...
        def remove_extra_bits_from_compressing(binary_string):
            bits_to_remove = int(binary_string[-3:], 2)
            binary_string = binary_string[:-8 - bits_to_remove]
            return binary_string

        def create_element_string_from_binary_string(binary_string):
            element_string_list = []
            binary_string = list(binary_string)
            while len(binary_string) > 0:
                element_list = self.node_tree.binary_to_element_string(binary_string, element_string_list)
            return element_list

        binary_string = extract_binary_string_from_file(input_file_path)
        self.decompress_file_path = check_type_of_file_from_binary_compressed_code(binary_string, input_file_path)
        binary_string = remove_extra_bits_from_compressing(binary_string)
        element_string = create_element_string_from_binary_string(binary_string)
